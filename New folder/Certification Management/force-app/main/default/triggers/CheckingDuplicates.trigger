    ...
    if ( Trigger.isInsert ) {
        for ( Certification_Request__c obj : Trigger.new ) {
            if ( obj.Voucher__c != NULL ) {
                List<Certification_Request__c> ob = [SELECT Certification__c, Voucher__c FROM Certification_Request__c WHERE Certification__c =: obj.Certification__c AND Voucher__c =: obj.Voucher__c];
                if ( !ob.isEmpty() ) {
